...
if (iconStatus === false) {
  ...
  let randomStickerNum = parseInt(
    stickerStartNum + Math.random() * stickerSum
  ).toString();
  icon =
    "#!icon=" +
    "https://github.com/KeiKinn/StickerOnScreen/raw/main/Stickers/Sticker_" +
    randomStickerNum +
    ".png";
}
!(async () => {
  ...
  if (body == null) {
    ...
    original = body
      .replace(/^ *(#|;|\/\/) */g, "#")
      .replace(/\x20.+url-and-header\x20/, " url ")
      .replace(/\x20+url\x20+/g, " url ")
      .replace(/(^[^#].+)\x20+\/\/.+/g, "$1")
      .split("\n");
    ...
    body.forEach((x, y, z) => {
      x = x
        .replace(/^ *(#|;|\/\/)/, "#")
        .replace(/\x20.+url-and-header\x20/, " url ")
        .replace(/\x20+url\x20+/, " url ")
        .replace(/^hostname\x20*=/, "hostname=")
        .replace(/(^[^#].+)\x20+\/\/.+/, "$1");
      //去掉注释
      if (Pin0 != null) {
      ...
      if (jsConverter != null) {
        for (let i = 0; i < jsConverter.length; i++) {
          ...
          if (x.indexOf(elem) != -1) {
            x = x.replace(
              /\x20(https?|ftp|file)(:\/\/.+\.js)/g,
              ` $1$2_script-converter-${jsctype}.js`
            );
          } else {
          }
        } //循环结束
        ...
        switch (type) {
          case " url script-":
            ...
            if (
              ...
              rebody = x.match(/\x20script[^\s]*(-body|-analyze)/)
                ? ", requires-body=true"
                : "";

              size = x.match(/\x20script[^\s]*(-body|-analyze)/)
                ? ", max-size=3145728"
                : "";
            ...
            if (isShadowrocket || isLooniOS || isLanceX || isEgern) {
              ...
              URLRewrite.push(
                x
                  .replace(/\x20{2,}/g, " ")
                  .replace(
                    /(^#)?(.*?)\x20url\x20(reject-200|reject-img|reject-dict|reject-array)/,
                    `${noteK}$2 - $3`
                  )
              );
            } else if (isSurgeiOS) {
              ...
              if (x.match(/dict$/)) {
                rejectType =
                  "https://raw.githubusercontent.com/mieqq/mieqq/master/reject-dict.json";
              } else if (x.match(/array$/)) {
                rejectType =
                  "https://raw.githubusercontent.com/mieqq/mieqq/master/reject-array.json";
              } else if (x.match(/200$/)) {
                rejectType =
                  "https://raw.githubusercontent.com/mieqq/mieqq/master/reject-200.txt";
              } else if (x.match(/img$/)) {
                rejectType =
                  "https://raw.githubusercontent.com/mieqq/mieqq/master/reject-img.gif";
              }
              MapLocal.push(
                x
                  .replace(/\x20{2,}/g, " ")
          ...
          case "-header ":
            let reHdType = x.match(" response-header ")
              ? "response"
              : "request";
            ...
            if (isLooniOS) {
              z[y - 1]?.match(/^#/) && script.push(z[y - 1]);
              script.push(
                `${noteK}http-${reHdType} ${reHdPtn} script-path=https://raw.githubusercontent.com/xream/scripts/main/surge/modules/replace-header/index.js, timeout=60, tag=replaceHeader_${y}, argument="${reHdArg1}->${reHdArg2}"`
              );
            } else if (isSurgeiOS || isLanceX || isShadowrocket || isEgern) {
              z[y - 1]?.match(/^#/) && script.push(z[y - 1]);
              script.push(
                `${noteK}replaceHeader_${y} = type=http-${reHdType}, pattern=${reHdPtn}, script-path=https://raw.githubusercontent.com/xream/scripts/main/surge/modules/replace-header/index.js, timeout=60, argument="${reHdArg1}->${reHdArg2}"`
              );
            } else if (isStashiOS) {
              z[y - 1]?.match(/^#/) && script.push("    " + z[y - 1]);
              script.push(
                `${noteK4}- match: ${reHdPtn}${noteKn6}name: replaceHeader_${y}${noteKn6}type: ${reHdType}${noteKn6}timeout: 30${noteKn6}argument: |-${noteKn8}${reHdArg1}->${reHdArg2}`
              );
              providers.push(
                `${noteK2}replaceHeader_${y}:${noteKn4}url: https://raw.githubusercontent.com/xream/scripts/main/surge/modules/replace-header/index.js${noteKn4}interval: 86400`
              );
            }
            break;
          ...
          case " echo-response ":
            ...
            if (/^(https?|ftp|file):\/\/.*/.test(arg)) {
              ...
              if (isLooniOS) {
                ...
                let mockPtn = x
                  .replace(/\x20{2,}/g, " ")
                  .split(" url echo-response")[0]
                  .replace(/^#/, "");

                let dataCon = x
                  .replace(/\x20{2,}/g, " ")
                  .split(" echo-response ")[2];
          ...
          default:
            if (type.match(/\x20(request|response)-body/)) {
              ...
              let reBdType = x.match(" response-body ")
                ? "response"
                : "request";
              ...
              let cronJs =
                x.split("://")[0].replace(/.+\x20([^\s]+)$/, "$1") +
                "://" +
                x.split("://")[1].split(",")[0];

              let croName = cronJs.substring(
                cronJs.lastIndexOf("/") + 1,
                cronJs.lastIndexOf(".")
              );
